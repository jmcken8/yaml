...
export default class Document {
  ...
  parse({
    ...
    directives.forEach(directive => {
      ...
      switch (name) {
        ...
        default:
          if (name) this.warnings.push(new YAMLWarning(directive, `YAML 1.2 only supports %TAG and %YAML directives, and not %${name}`));
      }
    ...
    contents.forEach(node => {
      if (node.valueRange && !node.valueRange.isEmpty) {
        if (contentNodes.length === 1) {
          this.errors.push(new YAMLSyntaxError(node, 'Document is not valid YAML (bad indentation?)'));
        }
    ...
    switch (contentNodes.length) {
      ...
      case 1:
        ...
        if (this.contents) {
          ...
          if (cb) {
            const cbNode = this.contents instanceof Collection && this.contents.items[0] ? this.contents.items[0] : this.contents;
  ...
  resolveTagName(node) {
    ...
    if (tag) {
      ...
      if (verbatim) {
        ...
        this.errors.push(new YAMLSemanticError(node, `Verbatim tags aren't resolved, so ${verbatim} is invalid.`));
        ...
        if (prefix) {
          ...
          this.errors.push(new YAMLSemanticError(node, `The ${handle} tag has no suffix.`));
        } else {
          this.errors.push(new YAMLSemanticError(node, `The ${handle} tag handle is non-default and was not declared.`));
        }
      }
    }
  ...
  resolveNode(node) {
    ...
    if (node.type === Type.ALIAS) {
      ...
      if (!src) {
        errors.push(new YAMLReferenceError(node, `Aliased anchor not found: ${name}`));
        return null;
  ...
  listNonDefaultTags() {
    return listTagNames(this.contents).filter(t => t.indexOf(defaultPrefix) !== 0);
  }
