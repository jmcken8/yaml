...
const isMap = ({
  type
}) => type === Type.FLOW_MAP || type === Type.MAP;

const isSeq = ({
  type
}) => type === Type.FLOW_SEQ || type === Type.SEQ;
...
export default class Schema {
  ...
  resolveNodeWithFallback(doc, node, tagName) {
    ...
    if (fallback) {
      doc.warnings.push(new YAMLWarning(node, `The tag ${tagName} is unavailable, falling back to ${fallback}`));
      const res = this.resolveNode(doc, node, fallback);
      res.tag = tagName;
      return res;
    } else {
      doc.errors.push(new YAMLReferenceError(node, `The tag ${tagName} is unavailable`));
    }
  ...
  stringifyProps(node, {
    ...
    const {
      tag
    } = node; // FIXME: should read prefix-skip from schema

    if (tag && tag.indexOf(defaultPrefix) !== 0) {
      const p = doc.tagPrefixes.find(p => tag.indexOf(p.prefix) === 0);
      props.push(p ? p.handle + tag.substr(p.prefix.length) : tag[0] === '!' ? tag : `!<${tag}>`);
    }
  ...
  stringify(item, ctx, onComment) {
    ...
    return props ? item instanceof Collection && !ctx.inFlow && item.items.length > 0 ? `${props}\n${ctx.indent}${str}` : `${props} ${str}` : str;
  }
